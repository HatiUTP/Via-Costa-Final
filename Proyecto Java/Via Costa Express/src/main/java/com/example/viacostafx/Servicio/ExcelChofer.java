...
public class ExcelChofer {

    public void exportChoferes(List<ChoferModel> choferes, String filePath) {
        ...
        titleCell.setCellValue("Tabla de Choferes - VIA COSTA");
        ...
        String[] headers = {"ID", "Nombre", "DNI", "Número de Licencia", "Teléfono", "Email", "ID Bus", "Activo"};
        ...
        // Agregar los datos
        // Parte del código donde se insertan datos de las filas:
        int rowNum = 3; // Comenzar en la fila 3
        for (ChoferModel chofer : choferes) {
            Row row = sheet.createRow(rowNum++);
            int colNum = 1; // Comenzar en columna B

            // ID
            Cell cell = row.createCell(colNum++);
            cell.setCellValue(chofer.getId());
            cell.setCellStyle(borderedCellStyle);

            // Nombre
            cell = row.createCell(colNum++);
            cell.setCellValue(chofer.getNombre());
            cell.setCellStyle(borderedCellStyle);

            // DNI
            cell = row.createCell(colNum++);
            try {
                cell.setCellValue(Double.parseDouble(chofer.getDni()));
            } catch (NumberFormatException e) {
                cell.setCellValue(chofer.getDni()); // Si no puede convertirse, escribirlo como texto.
            }
            cell.setCellStyle(borderedCellStyle);

            // Número de Licencia (como texto, ya que podría contener letras)
            cell = row.createCell(colNum++);
            cell.setCellValue(chofer.getNumeroLicencia());
            cell.setCellStyle(borderedCellStyle);

            // Teléfono
            cell = row.createCell(colNum++);
            try {
                cell.setCellValue(Double.parseDouble(chofer.getTelefono()));
            } catch (NumberFormatException e) {
                cell.setCellValue(chofer.getTelefono()); // Si no puede convertirse, escribirlo como texto.
            }
            cell.setCellStyle(borderedCellStyle);

            // Email
            cell = row.createCell(colNum++);
            cell.setCellValue(chofer.getEmail());
            cell.setCellStyle(borderedCellStyle);

            // ID Bus
            cell = row.createCell(colNum++);
            if (chofer.getBus() != null) {
                cell.setCellValue(chofer.getBus().getId());
            } else {
                cell.setCellValue(""); // Si no tiene bus asignado.
            }
            cell.setCellStyle(borderedCellStyle);

            // Activo
            cell = row.createCell(colNum++);
            cell.setCellValue(chofer.getIsActive() ? "Sí" : "No");
            cell.setCellStyle(borderedCellStyle);
        }
